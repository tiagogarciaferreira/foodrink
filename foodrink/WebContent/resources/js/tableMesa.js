...
function editar(objeto, parametro){
	 ...
	 document.getElementById("solicitacao").href = "editar-mesa?id="+idSelecionado;
	 document.getElementById("solicitacao").click();
}

function verificaRemover(valor){
	if(valor.value == 'SIM'){
		document.getElementById("solicitacao").href = "remover-mesa?id="+removerRegistro;
		document.getElementById("solicitacao").click();
...
function initTableMesa(){
	$('#tabelaMesa').DataTable({
		"autoWidth": true,
		"ordering":  false,
		"deferRender": true,
		"paging": true,
